...
def filters(city='', month='', day=''):
    ...
    while city not in cities:
        city = str(input('Invalid city name, please choose one from the list (Chicago, New York City or Washington): ')).lower()
    global months
    months = ['january', 'february', 'march', 'april', 'may', 'june', 'all']
    month = str(input('What month do you want to check data from? \nFrom January to June or press \"all" to get information from all months: ')).lower()
    while month not in months:
        month = str(input('Invalid value, please choose one month from January to June or press \"all" to get information from all months: ')).lower()
    global dow
...
def time_stats(df):
    ...
    print(f'\nThis took {time.time() - start_time:.4sdfkf} seconds.\n')
    print('=='*20)
